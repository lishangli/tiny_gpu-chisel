...
class GpuModuleImpl(outer: GpuLazyModule)(implicit p: Parameters)
  ...
  for (i <- 0 until NUM_CORES) {
    ...
    core.io.program_mem_read_valid := fetcher_read_valid(i)
    core.io.program_mem_read_address := fetcher_read_address(i)
    core.io.program_mem_read_ready := fetcher_read_ready(i)
    core.io.program_mem_read_data := fetcher_read_data(i)
    core.io.data_mem_read_valid := core_lsu_read_valid
    core.io.data_mem_read_address := core_lsu_read_address
    core.io.data_mem_read_ready := core_lsu_read_ready
    core.io.data_mem_read_data := core_lsu_read_data
    core.io.data_mem_write_valid := core_lsu_write_valid
    core.io.data_mem_write_address := core_lsu_write_address
